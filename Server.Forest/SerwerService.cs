...
public class SerwerService : BackgroundService
{
    ...
    private const string InputQueue = "chat-messages-server1";
    private const string OutputExchange = "chat-exchange-server1";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested && retryCount < maxRetries)
        {
            try
            {
                ...
                Console.WriteLine($"[Server1] Połączono z RabbitMQ.");
                break;
            }
            catch (BrokerUnreachableException)
            {
                retryCount++;
                Console.WriteLine($"[Server1 Retry] Nie udało się połączyć z RabbitMQ. Ponawiam próbę ({retryCount}/{maxRetries})...");
        ...
        if (_channel == null)
        {
            Console.WriteLine("[Server1] Nie udało się połączyć z RabbitMQ po wielu próbach. Serwis zostanie zamknięty.");
            return;
        }
        ...
        consumer.Received += (model, ea) =>
        {
            ...
            Console.WriteLine($"[Server1] Otrzymano: {message}");
            ...
            try
            {
                ...
                if (msgObj.Type == "Connect")
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(msgObj.PlayerDataSerialized))
                    {
                        ...
                        Console.WriteLine($"Gracz {newPlayer.Name} przeniósł się z {newPlayer.CurrentServer} na Server1.");
                        SendMessageToAll($"Gracz {newPlayer.Name} przeniósł się na Server1.");
                    }
                    else
                    {
                        // Nowe połączenie gracza
                        newPlayer = CreatePlayer(msgObj.PlayerName, msgObj.Class);
                        Console.WriteLine($"{newPlayer.Name} dołączył do gry jako {newPlayer.Class} na Server1.");
                        SendMessageToAll($"{newPlayer.Name} dołączył do gry jako {newPlayer.Class} na Server1.");
                    }

                    if (!players.ContainsKey(newPlayer.Name))
                    {
                        newPlayer.CurrentServer = "Server1";
                    ...
                    {
                        players[newPlayer.Name] = newPlayer;
                        SendMessageToAll($"Gracz {newPlayer.Name} ponownie połączył się z Server1.");
                    }
                ...
                else if (msgObj.Type == "Action")
                {
                    ...
                    {
                        Console.WriteLine($"[Server1] Nie jest tura gracza {player.Name} lub jest martwy. Akcja '{msgObj.Action}' zignorowana.");
                    ...
                    else if (msgObj.Action == "move")
                    {
                        ...
                        if (targetServer == "Server2")
                        {
                            ...
                            if (players.Remove(player.Name))
                            {
                                ...
                                Console.WriteLine($"Gracz {player.Name} usunięty z Server1 po przeniesieniu.");
    ...
    private void SendPlayerMoveToOtherServer(PlayerData player, string targetServer)
    {
        ...
        string targetQueue = targetServer == "Server1" ? "chat-messages-server1" : "chat-messages-server2";
        ...
        Console.WriteLine($"[Server1] Gracz {player.Name} został przekazany do {targetServer}");
    }
